...
('RM-005', 'Cokelat Batang', 'raw', 'kg', 60000)
	`)
	if err != nil {
		log.Fatal("Seed products error:", err)
	}
...
(1, 6, 0.500), (1, 9, 0.010),
(2, 6, 0.300), (2, 10, 0.050),
(3, 8, 0.200), (3, 6, 0.400),
(4, 6, 0.200), (4, 7, 0.020),
(5, 6, 0.600), (5, 9, 0.020);
	`)
	if err != nil {
		log.Fatal("Seed materials error:", err)
	}
...
('Hadi', 'hadi@food.com', 'pass9', 'active'), ('Yati', 'yati@food.com', 'pass10', 'active');
	`)
	if err != nil {
		log.Fatal("Seed operators error:", err)
	}
...
('PO-009', 4, 200, 'planned'), ('PO-010', 5, 100, 'planned');
	`)
	if err != nil {
		log.Fatal("Seed production orders error:", err)
	}
...
(1, 4, 9, 49, 0), (2, 5, 10, 97, 1);
	`)
	if err != nil {
		log.Fatal("Seed production logs error:", err)
	}
...
(3, 3, 6, '2024-05-02 09:30', '2024-05-02 11:00'), (4, 5, 8, '2024-05-02 12:00', '2024-05-02 13:00');
	`)
	if err != nil {
		log.Fatal("Seed machine logs error:", err)
	}
...
(4, 6, 'OUT', 30), (4, 7, 'OUT', 3);
	`)
	if err != nil {
		log.Fatal("Seed inventory transactions error:", err)
	}
...
(7, 0, 0, 0.00, 0), (8, 0, 0, 0.00, 0), (9, 0, 0, 0.00, 0), (10, 0, 0, 0.00, 0);
	`)
	if err != nil {
		log.Fatal("Seed production summaries error:", err)
	}
